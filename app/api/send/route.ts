...
export async function POST(req: NextRequest) {
  try {
    ...
    if (chain === 'ETH') {
      ...
      return NextResponse.json({ success: true, chain: 'ETH', status: 'prepared', tx: { from, to, value: amountWei, note: 'Sign and broadcast via MetaMask or Alchemy Smart Wallet' } })
    }
    if (chain === 'SOL') {
      ...
      return NextResponse.json({ success: true, chain: 'SOL', status: 'prepared', tx: { from, to, lamports, blockhash, note: 'Transaction prepared. Sign with wallet to broadcast.' } })
    }
